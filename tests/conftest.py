...
BASE_URL = os.getenv(f"{ADAPTER_NAME.upper()}_BASE_URL", "https://api.example.com/k3cloud/")
...
def standard_auth_config() -> dict[str, str]:
    ...
    return {
        "acct_id": os.getenv(f"{prefix}_ACCT_ID", "test_acct_id"),
        "username": os.getenv(f"{prefix}_USERNAME", "administrator"),
        "app_id": os.getenv(f"{prefix}_APP_ID", "test_app_id"),
        "app_secret": os.getenv(f"{prefix}_APP_SECRET", "test_app_secret"),
        "lcid": os.getenv(f"{prefix}_LCID", "2052"),
        "base_url": BASE_URL,
    }
